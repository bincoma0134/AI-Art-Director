...
def main():
    ...
    print(f"🔥 BẮT ĐẦU HUẤN LUYỆN: {device} | MÔ HÌNH: EfficientNet-B3 (Early Stopping Enabled)")

    BATCH_SIZE = 32
    EPOCHS = 20
    LEARNING_RATE = 1e-4
    PATIENCE = 5 # Dừng nếu sau 5 epoch không cải thiện
    ...
    model = AI_ArtDirector_EfficientNet(num_attributes=12).to(device)
    criterion = nn.MSELoss()
    # Tăng weight_decay từ 1e-4 lên 5e-3 để ép Regularization mạnh hơn
    ...
    for epoch in range(EPOCHS):
        ...
        if epoch_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), os.path.join(BASE_DIR, "efficientnet_b3_best.pth"))
        else:
            trigger_times += 1
            print(f"⚠️ Val Loss không giảm. Cảnh báo Early Stopping: {trigger_times}/{PATIENCE}")
            if trigger_times >= PATIENCE:
                print("\n🛑 EARLY STOPPING KÍCH HOẠT! Dừng huấn luyện để chống Overfitting.")
                break
    ...
    # 4. DATA EXPORT
    # ==========================================
    with open(os.path.join(LOGS_DIR, 'efficientnet_b3_history.json'), 'w') as f:
        json.dump(history, f)
        
    np.savez(os.path.join(LOGS_DIR, 'efficientnet_b3_predictions.npz'), actual=best_labels, predicted=best_preds)
    print(f"💾 Dữ liệu trích xuất đã được lưu. Hãy chạy visualize_efficientnet_b3.py!")
