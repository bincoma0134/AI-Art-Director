...
def main():
    ...
    print(f"🔥 BẮT ĐẦU HUẤN LUYỆN: {device} | MÔ HÌNH: MobileNetV2 (Early Stopping Enabled)")
    ...
    model = AI_ArtDirector_MobileNetV2(num_attributes=12).to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        if epoch_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), os.path.join(BASE_DIR, "mobilenet_v2_best.pth"))
        else:
            trigger_times += 1
    ...
    print(f"🎉 HUẤN LUYỆN MOBILENET-V2 HOÀN TẤT trong {total_time/60:.2f} phút!")

    # ==========================================
    # 4. DATA EXPORT (LOGGING FOR VISUALIZATION)
    # ==========================================
    with open(os.path.join(LOGS_DIR, 'mobilenet_v2_history.json'), 'w') as f:
        json.dump(history, f)
        
    np.savez(os.path.join(LOGS_DIR, 'mobilenet_v2_predictions.npz'), actual=best_labels, predicted=best_preds)
    print(f"💾 Dữ liệu trích xuất đã được lưu. Hãy chạy visualize_mobilenet_v2.py!")
