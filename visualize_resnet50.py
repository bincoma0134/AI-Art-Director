...
MODEL_COLOR = '#38bdf8' # Nhận diện thương hiệu FOXIL (ResNet: Xanh dương)

def load_data():
    history_path = os.path.join(LOGS_DIR, 'resnet50_history.json')
    preds_path = os.path.join(LOGS_DIR, 'resnet50_predictions.npz')
    
    if not os.path.exists(history_path) or not os.path.exists(preds_path):
        print("❌ Lỗi: Không tìm thấy file dữ liệu (JSON/NPZ). Hãy chạy train_resnet50.py trước!")
        sys.exit(1)
...
def plot_learning_curve(history):
    ...
    plt.title('ResNet-50: Learning Curve (Early Stopping)', fontsize=14, fontweight='bold')
    plt.xlabel('Epochs', fontsize=12)
    plt.ylabel('Loss', fontsize=12)
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.legend(fontsize=12)
    plt.savefig(os.path.join(LOGS_DIR, 'resnet50_01_learning_curve.png'), dpi=300, bbox_inches='tight')
    plt.close()
    print("✅ Đã xuất: resnet50_01_learning_curve.png")
...
def plot_scatter(actual, predicted, pcc_score, srcc_score, r2_score):
    ...
    title_text = (f'ResNet-50: Actual vs Predicted (Overall)\n'
    ...
    plt.legend()
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.savefig(os.path.join(LOGS_DIR, 'resnet50_02_scatter_plot.png'), dpi=300, bbox_inches='tight')
    plt.close()
    print("✅ Đã xuất: resnet50_02_scatter_plot.png")
...
def plot_radar(actual, predicted):
    ...
    ax.plot(angles, pred_sample, linewidth=2, linestyle='solid', label='AI ResNet-50', color=MODEL_COLOR)
    ...
    plt.savefig(os.path.join(LOGS_DIR, 'resnet50_03_radar_chart.png'), dpi=300, bbox_inches='tight')
    plt.close()
    print("✅ Đã xuất: resnet50_03_radar_chart.png")

if __name__ == '__main__':
    print("🎨 Đang trích xuất dữ liệu nâng cao ResNet-50...")
